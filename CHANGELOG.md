...
The G6 sabotage gate, the last item that had been sitting in "what's next" since phase 10. The HR Portal now reads an `X-Sabotage: leave.submit` header and silently no-ops the write while still returning the same success response — a realistic "looks fine, did nothing" bug. G6 re-runs an already-passing candidate once more with that header set (via `SABOTAGE` env → Playwright's `extraHTTPHeaders`) and requires it to fail; a test that still passes gets a new failure class, `WEAK_ASSERTION`, and specific retry feedback ("assert on the real effect, not the success message"). G6 only runs when static analysis shows the candidate actually calls the sabotaged method, reusing the same `CodeAnalysis` mechanism G2 already relies on — sabotaging an unrelated test would prove nothing. Added a 13th scenario, `sabotage-survived` (weak candidate on attempt 1, corrected on attempt 2), and confirmed live that the three existing scenarios reaching `PENDING_REVIEW` now also pass G6 for real, not just the new one — their gate-report length grew from 6 to 7 entries, which the integration test's assertion had to be made to expect rather than hard-code. See [ADR-0006](docs/adr/0006-sabotage-gate-scoped-to-one-feature.md) for why this is one named feature, not a generic mutation-testing framework.
