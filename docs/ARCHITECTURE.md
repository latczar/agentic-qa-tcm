...
- Sabotage flags (see gate G6): an `X-Sabotage: leave.submit` header makes leave submission silently fail to persist while still returning success, so a test that still passes has proved nothing. Built in phase 12; currently the only sabotage-able feature.
...
- **Run state machine** with Postgres persistence (`pipeline` schema). Tables: `generation_runs`, `generation_attempts`. The review decision (reviewer, timestamp, comment) lives as three columns on `generation_runs` rather than a separate `review_decisions` table: a run has exactly one decision, so a side table would only add a join for no benefit — the same reasoning that already puts each attempt's gate report in a `jsonb` column instead of its own table. Unique index on `(test_case_id, test_case_version)`.
...
| G6   | Negative control        | only when the candidate touches a sabotage-able feature (currently `leave.submit`): run once more with that feature's sabotage flag on; the test must fail                                                                                                                                                                                         | assertions that do not actually observe the outcome                 |
...
| Useless assertion that passes       | fixture with `expect(true).toBe(true)` and no state matcher                                           | G4 (and G6 for `leave.submit`)    | retry with assertion guidance                               |
...
| 3     | Mock TCM                             | schema, TestRail-shaped API, atomic claim, versioning, minimal UI, 20 seeded manual cases                                                                                                                        | 9 integration tests against Postgres incl. concurrent claim; 31 seeded cases; UI flips status and bumps version                                                                                                                                                                                     |
| 4     | Manifest and MCP                     | ts-morph extractor, manifest drift check, MCP server with seven tools, tested with MCP Inspector, wired into Claude Code for a demo                                                                              | 12 page objects, 104 methods in the manifest; extractor tests incl. determinism; 11 MCP protocol tests; stdio probe; drift check in CI                                                                                                                                                              |
...
| 9     | CI hardening                         | badges, a manual (`workflow_dispatch`-only) Ollama bench job that never gates a merge, `docker compose config` validation kept in `checks`                                                                       | verified against a real GitHub Actions run on first push, not just the YAML: `checks` (39s), `framework-e2e` (41s) and `integration` (139s) all green, `bench-ollama` correctly skipped (push event, not a manual dispatch), `playwright-report` and `pipeline-artifacts` downloadable from the run |
| 10    | Polish                               | README rewrite (pitch, diagram, "why not prompt-to-code", quickstart, demo GIF, a 12-row failure gallery verified against the real `scenario.json` fixtures, honest limitations, what's next), 4 ADRs, CHANGELOG | repo reads well cold; demo GIF recorded live against the real stack, not staged; stretch items (G6 sabotage gate, TCM MCP server, branch-based review) intentionally left for "what's next" rather than built                                                                                       |
| 11    | Deferred-run watcher (stretch)       | `POST /runs/:id/retry`, a third n8n workflow polling for runs stuck `DEFERRED` past 10 minutes                                                                                                                   | verified live against real Postgres and n8n: found 4 genuinely stale runs left from earlier testing, retried all 4, confirmed via the `deferrals` counter and Mailpit                                                                                                                               |
| 12    | G6 sabotage gate (stretch)           | `X-Sabotage: leave.submit` header in the HR Portal, gate G6, a 13th scenario (`sabotage-survived`)                                                                                                               | verified live: a weak candidate (checks only the success message) is caught by G6 and retried; a strong one (checks the real outcome) passes; all 3 existing scenarios that reach `PENDING_REVIEW` now also pass G6 for real, not just the new one                                                  |
...
- **Docs**: `ARCHITECTURE.md`, ADRs for the decisions a reviewer would question (n8n as glue not brain, no embeddings, replay provider, manifest as the single source of truth, sabotage gate scoped to one feature).
