...
## Phase 12: the sabotage gate

**How does the sabotage actually work — is it a special test mode, a mock, a feature flag service?**
None of those — it's four lines in the real `POST /leave` handler. If the request carries `X-Sabotage: leave.submit`, the handler skips the one line that writes to the store, but still returns the exact same success response. That's deliberate: the bug this gate hunts for is "the write silently didn't happen but the response looks identical," which is a real production bug shape, not a contrived test artefact. The header only ever gets set by Playwright's config when the orchestrator sets a `SABOTAGE` env var for that one run — there's no code path a real client request could trigger it through by accident.

**Why does G6 only run for tests that touch leave submission, and not every generated test?**
Because sabotaging a feature a test never exercises proves nothing in either direction — the test would pass regardless, and G6 would just be adding 30 seconds of Playwright time for no signal. G6 checks the same `CodeAnalysis.appRefs` structure G2 already builds for symbol-existence checking (does the code call `app.leaveForm.submitRequest`?) before deciding whether to run at all. Reusing an existing, already-tested mechanism for relevance-checking was more important than building something new — the interesting engineering here is the negative control itself, not a second symbol-scanner.

**Turning on a new gate for candidates that already exist sounds risky — what actually broke?**
Nothing broke, but something had to be found and fixed to prove that: three scenarios that already reached `PENDING_REVIEW` (`happy`, `malformed-then-valid`, `execution-failure`) all happen to touch leave submission with a real assertion already, so G6 now runs for them too and correctly passes. Their gate report grew from 6 entries to 7. The integration test had a hard-coded `toEqual(['G0'..'G5'])` check that would have failed for all three the moment G6 turned on — caught by actually running the full suite, not just the new scenario, and fixed by making the assertion accept either 6 or 7 gates rather than special-casing three files that didn't ask to be touched.

**What would you say if someone asked "isn't this just mutation testing"?**
It's the same idea in miniature, not the general-purpose version. Real mutation testing tools instrument a whole codebase and generate many mutants automatically; G6 sabotages exactly one named, hand-chosen failure mode. That's a scoping decision, written down in [ADR-0006](adr/0006-sabotage-gate-scoped-to-one-feature.md): a real negative control for one realistic bug shape is more valuable to prove the concept than a generic framework that would take much longer to build and be harder to reason about. The honest limitation that comes with that is also written down: any other feature could have the exact same "checks only the surface response" flaw today, with nothing to catch it.
